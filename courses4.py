...
# Physics constants
repulsionConstant = 2000000.0
electronRepulsionConstant = 400000.0
hookesLawK = 0.2
...
class Coarse(object):
    ...
    def getBlurb(self):
        ...
        return "%s\n%s (%d)\n\nPrerequisites: %s" % (courseNumberAsStr, self.courseName, self.units, self.getPrereqsAsString())
...
class EventBasedAnimationDemo(EventBasedAnimationClass):
    ...
    def onKeyPressed(self, event):
        if event.keysym == "space": self.addRandomCourse()
        	...
        	for electron in self.electrons:
        		electron.x, electron.y = cx,cy
        elif event.char == "t":
            self.colourScheme = "progress" if self.colourScheme == "semester" else "semester"
        ...
        for i in xrange(len(self.courses)):
            ...
            for prereq in course.prereqs:
                ...
                try:
                    ...
                    self.canvas.create_line(x0+newr*cos(theta),y0+newr*sin(theta),x1-newr*cos(theta),y1-newr*sin(theta),arrow=FIRST)
                except:
                    prereqsMissing = True

            if self.colourScheme == "progress":
                ...
                elif prereqsFulfilled:
                    colour = "orange"

                outlineColour = "black"
                if prereqsMissing or dictionaryOfAllCourses[course.courseNumber]["fall"] == 0 and dictionaryOfAllCourses[course.courseNumber]["spring"] == 1: outlineColour = "red"
                ...
                if dictionaryOfAllCourses[course.courseNumber]["fall"] == 1 and dictionaryOfAllCourses[course.courseNumber]["spring"] == 0:
                    colour = "orange"
                elif dictionaryOfAllCourses[course.courseNumber]["fall"] == 0 and dictionaryOfAllCourses[course.courseNumber]["spring"] == 1:
                    colour = "light green"
                ...
                if prereqsMissing:
                    ...
                    if course.status == 1: outlineColour = "green"
                    elif course.status == 2: outlineColour = "yellow"
                    else:
                        if prereqsFulfilled:
                            outlineColour = "orange"
                        else: outlineColour = "black"

            
            self.canvas.create_oval(x0-newr,y0-newr,x0+newr,y0+newr,fill=colour,outline=outlineColour,width=2,activefill=activefillcolour,tags="course")
    ...
    def drawLegend(self):
        ...
        self.canvas.create_text(x2,y,text="Spring only",fill="white",anchor=W, width = 130)
        y += 2.5*r
        self.canvas.create_oval(x-r,y-r,x+r,y+r,fill="green",width=2)
        self.canvas.create_text(x2,y,text="Taken",fill="white",anchor=W, width = 130)
        y += 2.5*r
        self.canvas.create_oval(x-r,y-r,x+r,y+r,fill="yellow",width=2)
        self.canvas.create_text(x2,y,text="Taking",fill="white",anchor=W, width = 130)
        y += 2.5*r
        self.canvas.create_oval(x-r,y-r,x+r,y+r,fill="orange",width=2)
        self.canvas.create_text(x2,y,text="Eligible",fill="white",anchor=W, width = 130)
